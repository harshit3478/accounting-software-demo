      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">
            Manage deleted documents. Documents are automatically deleted after
            30 days.
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-6 flex justify-between items-center">
          <div className="flex items-center space-x-4">
            ...
            {expiredDocs.length > 0 && (
              <span className="text-sm text-red-600 font-medium">
                ...
                {expiredDocs.length !== 1 ? "s" : ""} past 30 days
              </span>
            )}
          </div>
          ...
          >
            {processingId === -1
              ? "Cleaning up..."
              : `Cleanup Expired (${expiredDocs.length})`}
          </button>
        </div>
                        ...
                        <div>
                          ...
                          {doc.type === "folder" && doc.folderContents && (
                            <div className="text-xs text-gray-500">
                              ...
                              {doc.folderContents.files} files,{" "}
                              {doc.folderContents.folders} folders)
                            </div>
                          )}
            ...
            >
              ...
              <div className="p-6">
                <p className="text-sm text-gray-600">
                  {confirmModal.type === "cleanup" &&
                    `This will permanently delete ${expiredDocs.length} document(s) that are older than 30 days. This action cannot be undone.`}
                  {confirmModal.type === "recover" && confirmModal.doc && (
                    <>
                      ...
                      {confirmModal.doc.type === "folder" ? (
                        <>
                          {" "}
                          The folder will be restored to root level (contents
                          not restored).
                        </>
                      ) : (
                        <>
                          {" "}
                          It will be restored to your documents with [RECOVERED]
                          prefix.
                        </>
                      )}
                    </>
                  ...
                  {confirmModal.type === "delete" && confirmModal.doc && (
                    <>
                      ...
                      {confirmModal.doc.type === "folder" &&
                      confirmModal.doc.folderContents ? (
                        <>
                          {" "}
                          This will remove the folder record and all its
                          contents ({confirmModal.doc.folderContents.totalItems}{" "}
                          items). This action cannot be undone.
                        </>
                      ) : (
                        <>
                          {" "}
                          This will remove the file from storage. This action
                          cannot be undone.
                        </>
                      )}
                    </>
