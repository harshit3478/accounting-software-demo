  ...
  try {
    ...
    const response = {
      ...
      metadata: {
        ...
        folderCount: folder.children.filter(
          (c) => c.type === DocumentType.folder,
        ).length,
        fileCount: folder.children.filter((c) => c.type === DocumentType.file)
          .length,
      },
    };
...
export async function PATCH(
  ...
  try {
    ...
    console.log(
      `✅ Folder renamed: "${folder.name}" → "${updated.name}" by ${user.name}`,
    );
...
export async function DELETE(
  ...
  try {
    ...
    const fileCount = allDescendants.filter(
      (d) => d.type === DocumentType.file,
    ).length;
    const folderCount = allDescendants.filter(
      (d) => d.type === DocumentType.folder,
    ).length;
