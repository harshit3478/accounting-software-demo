...
export async function POST(
  ...
  try {
    ...
    if (deletedDoc.type === DocumentType.folder) {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        const sortedDescendants = [...descendants].sort((a, b) => {
          ...
          const getDepth = (item: any): number => {
            ...
            while (
              currentParentId !== deletedDoc.originalDocId &&
              currentParentId !== null
            ) {
              depth++;
              const parent = descendants.find(
                (d: any) => d.id === currentParentId,
              );
              if (!parent) break;
        ...
        for (const desc of sortedDescendants) {
          ...
          if (desc.parentId === deletedDoc.originalDocId) {
            ...
            console.warn(
              `Skipping ${desc.name}: parent ${desc.parentId} not found`,
            );
            continue;
          }
      ...
      const fileCount = descendants.filter(
        (d: any) => d.type === DocumentType.file,
      ).length;
      const folderCount = descendants.filter(
        (d: any) => d.type === DocumentType.folder,
      ).length;
    ...
    if (deletedDoc.type === DocumentType.folder) {
      ...
      if (deletedDoc.folderContents) {
        ...
        for (const item of descendants) {
          ...
          if (
            (item.type === "file" || item.type === DocumentType.file) &&
            item.fileName
          ) {
            try {
              ...
              console.error(
                `Failed to delete nested file ${item.fileName} from R2:`,
                r2Error,
              );
