...
function verifyWebhookSignature(payload: string, signature: string): boolean {
  ...
  if (!verifier || verifier === "your_webhook_verifier_token_here") {
    console.warn(
      "QUICKBOOKS_WEBHOOK_VERIFIER not configured - skipping signature verification in development",
    );
...
async function processPaymentEvent(entity: any, event: any) {
  ...
  try {
    ...
    if (existingPayment) {
      ...
      console.log(
        `Payment ${existingPayment.id} already exists for QB payment ${qbPaymentId}`,
      );
      return {
        success: true,
        action: "updated",
    ...
    try {
      ...
      if (qbo) {
        ...
        console.warn(
          "Could not create QuickBooks client, using webhook payload data",
        );
        paymentData = extractPaymentDataFromWebhook(entity, event);
      }
    } catch (error: any) {
      console.warn(
        "Failed to fetch from QuickBooks API, using webhook payload:",
        error.message,
      );
      paymentData = extractPaymentDataFromWebhook(entity, event);
    }
    ...
    try {
      const methodId =
        methodMap.get(paymentData.methodName.toLowerCase()) ||
        methodMap.get("cash")!;
      ...
      console.log(
        `Created payment ${payment.id} from QuickBooks payment ${qbPaymentId}`,
      );
      return {
        success: true,
        action: "created",
      ...
      if (
        ...
        console.log(
          `Duplicate detected for QB payment ${qbPaymentId}, treating as success`,
        );
...
async function fetchPaymentFromQuickBooks(
  qbo: any,
  paymentId: string,
): Promise<{
  amount: number;
...
function extractPaymentDataFromWebhook(
  ...
  const customerName =
    entity.customerName || event.customerRef?.name || "Unknown Customer";
...
export async function GET() {
  const verifierConfigured =
    ...
    process.env.QUICKBOOKS_WEBHOOK_VERIFIER !==
      "your_webhook_verifier_token_here";
