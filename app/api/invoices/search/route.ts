...
export async function GET(request: NextRequest) {
  try {
    ...
    const requestedStatuses = searchParams.get("status")?.split(",") || [
      "pending",
      "partial",
      "overdue",
    ];
    const showHeldInvoices = requestedStatuses.includes("hold");
    ...
    // Build where clause
    const where: any = {
      isHold: showHeldInvoices ? true : false,
    };
                ...
                (installment: any) => ({
                  ...
                  paidAmount:
                    installment.paidAmount != null
                      ? installment.paidAmount.toNumber()
                      : null,
                }),
              ),
            }
