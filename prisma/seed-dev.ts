  ...
  // 1. Paid invoice — Sarah Johnson
  {
    ...
    const inv = await prisma.invoice.create({
      data: {
        ...
        status: "paid",
        items: [
          { name: "18K Gold Necklace 24g", quantity: 1, pricePerItem: 1850 },
        ],
  ...
  // 2. Partial invoice — Mohammed Al-Rashid
  {
    ...
    const inv = await prisma.invoice.create({
      data: {
        ...
        status: "partial",
        items: [
          { name: "18K Gold Bracelet 40g", quantity: 1, pricePerItem: 3200 },
        ],
        termsSnapshot: defaultTerms.lines,
        source: "manual",
      },
  ...
  // 4. Pending invoice — David Chen
  {
    ...
    const inv = await prisma.invoice.create({
      data: {
        ...
        status: "pending",
        items: [
          { name: "18K Gold Earrings 16g", quantity: 2, pricePerItem: 1200 },
        ],
        termsSnapshot: defaultTerms.lines,
        source: "manual",
      },
  ...
  // 5. Paid invoice — Fatima Hassan (via Zelle)
  {
    ...
    const inv = await prisma.invoice.create({
      data: {
        ...
        items: [
          {
            name: "18K Gold Set (Necklace + Earrings) 60g",
            quantity: 1,
            pricePerItem: 5000,
          },
        ],
  ...
  // 6. Partial invoice — Robert Williams (bank transfer)
  {
    ...
    const inv = await prisma.invoice.create({
      data: {
        ...
        status: "partial",
        items: [
          { name: "18K Gold Bangle 20g", quantity: 1, pricePerItem: 1600 },
        ],
        termsSnapshot: defaultTerms.lines,
        source: "manual",
      },
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const inv = await prisma.invoice.create({
      data: {
        ...
        status: "paid",
        items: [
          { name: "18K Gold Chain 15g", quantity: 1, pricePerItem: amount },
        ],
        termsSnapshot: defaultTerms.lines,
        source: "manual",
      },
