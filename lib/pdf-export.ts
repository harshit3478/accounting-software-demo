...
export function generateInvoicesPDF(invoices: Invoice[], filters?: {
...
export function generateSingleInvoicePDF(invoice: Invoice) {
  ...
  doc.text(`$${invoice.paidAmount.toFixed(2)}`, 170, totalsY + 7, { align: 'right' });
  ...
  doc.text(`$${(invoice.amount - invoice.paidAmount).toFixed(2)}`, 170, totalsY + 17, { align: 'right' });
