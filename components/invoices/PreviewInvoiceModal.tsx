    ...
    >
      <div className="space-y-6">
        ...
        <div className="bg-blue-50 p-4 rounded-lg">
          <h4 className="text-lg font-semibold text-gray-900 mb-2">Invoice Summary</h4>
          <p className="text-sm text-gray-600">Please review the invoice details before creating.</p>
        </div>
        ...
        <div className="space-y-2 bg-gray-50 p-4 rounded-lg">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-medium text-gray-900">${getTaxAmount().toFixed(2)}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-600">
              Discount {discountType === 'percentage' ? `(${discount.toFixed(2)}%)` : ''}:
            </span>
        ...
        {isLayaway && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
            <p className="text-sm text-yellow-800">
              <span className="font-semibold">⚠️ Layaway Invoice:</span> This invoice is marked as an installment payment plan.
            </p>
          </div>
        )}
      </div>
