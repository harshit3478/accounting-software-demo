...
export default function EditPaymentModal({
  ...
  useEffect(() => {
    ...
    fetch("/api/payment-methods")
      ...
      .then((data) => {
        const activeMethods = (data || []).filter(
          (m: PaymentMethodType) => m.isActive,
        );
    ...
    fetch(
      "/api/invoices?status=all&limit=200&sortBy=invoiceNumber&sortDirection=desc",
    )
  ...
  const matchedInvoiceSummary = hasMatchedSplits
    ? payment
        .paymentMatches!.map(
          (m) => `${m.invoice.invoiceNumber} (${m.invoice.clientName})`,
        )
        .join(", ")
    : "";
          ...
          {hasMatchedSplits ? (
            <>
              ...
              <p className="mt-1 text-xs text-amber-700">
                Invoice cannot be changed for matched payments. Unmatch and
                relink first.
              </p>
            </>
          ) : (
            ...
            >
              ...
              {invoices.map((inv) => (
                <option key={inv.id} value={inv.id}>
                  {inv.invoiceNumber} - {inv.clientName} (Remaining $
