...
async function main() {
  ...
  for (const m of methodDist) {
    ...
    console.log(
      `  ${name}: ${m._count} payments, $${Number(m._sum.amount || 0).toLocaleString("en-US", { minimumFractionDigits: 2 })}`,
    );
  }
  ...
  console.log(`\nPayment matching:`);
  console.log(
    `  Matched: ${matchedPayments} (${((matchedPayments / paymentCount) * 100).toFixed(1)}%)`,
  );
  console.log(
    `  Unmatched: ${unmatchedPayments} (${((unmatchedPayments / paymentCount) * 100).toFixed(1)}%)`,
  );

  // Layaway stats
  if (layawayPlanCount > 0) {
    const cancelledPlans = await prisma.layawayPlan.count({
      where: { isCancelled: true },
    });
    const paidInstallments = await prisma.layawayInstallment.count({
      where: { isPaid: true },
    });
    console.log(`\nLayaway:`);
    ...
    console.log(
      `  Paid installments: ${paidInstallments} (${((paidInstallments / installmentCount) * 100).toFixed(1)}%)`,
    );
  }
  ...
  for (const inv of sampleInvoices) {
    ...
    console.log(
      `  ${inv.invoiceNumber} | ${inv.clientName} | $${Number(inv.amount)} | Paid: $${paidTotal} | Status: ${inv.status} | Layaway: ${inv.isLayaway}`,
    );
    if (inv.layawayPlan) {
      console.log(
        `    Plan: ${inv.layawayPlan.months}mo, ${inv.layawayPlan.paymentFrequency}, ${inv.layawayPlan.installments.length} installments`,
      );
    }
  }
  ...
  console.log(
    `  Invoices with negative amount: ${negativeInvoices} ${negativeInvoices > 0 ? "⚠️" : "✓"}`,
  );
  ...
  console.log(
    `  Payments with negative amount: ${negativePayments} ${negativePayments > 0 ? "⚠️" : "✓"}`,
  );
  ...
  console.log(
    `  Orphaned payments: ${orphanedPayments} ${orphanedPayments > 0 ? "⚠️" : "✓"}`,
  );
  ...
  console.log(
    `  Duplicate external invoice numbers: ${extNums.length} ${extNums.length > 0 ? "⚠️" : "✓"}`,
  );
