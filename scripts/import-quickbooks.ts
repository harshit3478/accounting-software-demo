...
if (!csvPath) {
  console.error(
    "Usage: npx tsx scripts/import-quickbooks.ts --csv=<path> --userId=<id> [--dry-run] [--limit=N]",
  );
  process.exit(1);
}
...
function parseLayawayMemo(memo: string): LayawayPlanData | null {
  ...
  const scheduleRegex =
    /(\d{2}\/\d{2}\/\d{2})\s*-\s*(.+?)(?:\(Paid\s+\$?([\d,.]+)\s+on\s+(\d{2}\/\d{2}\/\d{2})\)|[–-]\s*\$?([\d,.]+))/g;
  let match;
  ...
  while ((match = scheduleRegex.exec(memo)) !== null) {
    ...
    const paidAmount = paidAmountStr
      ? parseFloat(paidAmountStr.replace(",", ""))
      : null;
    const paidDate = paidDateStr ? parseShortDate(paidDateStr) : null;
    const amount =
      paidAmount ||
      (scheduledAmountStr
        ? parseFloat(scheduledAmountStr.replace(",", ""))
        : 0);
...
function detectPaymentMethod(accountName: string): string {
  ...
  if (lower.includes("bank of america") || lower.includes("boa"))
...
async function main() {
  ...
  for (const [txnId, txnRows] of txnGroups) {
    ...
    const salesRows = txnRows.filter((r) =>
      r["Account Name"]?.includes("Sales"),
    );
    const arDebitRow = txnRows.find(
      (r) =>
        r["Account Name"] === "Accounts Receivable" &&
        parseFloat(r["Debit Amount (Two Column Approach)"] || "0") > 0,
    );
    const arCreditRow = txnRows.find(
      (r) =>
        r["Account Name"] === "Accounts Receivable" &&
    ...
    if (salesRows.length > 0 && arDebitRow) {
      // This is an invoice
      const customer =
        arDebitRow["Customer"] || txnRows[0]["Customer"] || "Unknown";
      customerNames.add(customer);

      const invoiceNum = arDebitRow["Invoice Number"] || "";
      const amount = parseFloat(
        arDebitRow["Debit Amount (Two Column Approach)"] || "0",
      );
      const dateAdded = parseCSVDate(arDebitRow["Transaction Date Added"]);
      const memo =
        arDebitRow["Notes / Memo"] ||
        txnRows.find((r) => r["Notes / Memo"])?.["Notes / Memo"] ||
        "";
      ...
      for (const sr of salesRows) {
        const desc =
          sr["Transaction Line Description"] ||
          sr["Transaction Description"] ||
          "";
        // Format: "CustomerName - InvoiceNo - ItemName"
        const parts = desc.split(" - ");
        const itemName = parts.length >= 3 ? parts.slice(2).join(" - ") : desc;
        const lineAmount = Math.abs(
          parseFloat(
            sr["Amount (One column)"] ||
              sr["Credit Amount (Two Column Approach)"] ||
              "0",
          ),
        );
      ...
      const isLayaway =
        memo.toLowerCase().includes("lay-away") ||
        memo.toLowerCase().includes("layaway");
      ...
      const customer =
        arCreditRow["Customer"] || txnRows[0]["Customer"] || "Unknown";
      customerNames.add(customer);

      const amount = parseFloat(
        arCreditRow["Credit Amount (Two Column Approach)"] || "0",
      );
      ...
      const otherAccount =
        arCreditRow["Other Accounts for this Transaction"] || "";
  ...
  console.log(
    `  Layaway invoices: ${invoices.filter((i) => i.isLayaway).length}`,
  );
  ...
  if (dryRun) {
    ...
    for (const inv of invoices.slice(0, 5)) {
      console.log(
        `  ${inv.externalInvoiceNumber} | ${inv.customerName} | $${inv.amount} | Items: ${inv.items.length} | Layaway: ${inv.isLayaway}`,
      );
    }
    console.log("\nSample payments (first 5):");
    for (const pay of payments.slice(0, 5)) {
      console.log(
        `  ${pay.invoiceNumber || "N/A"} | ${pay.customerName} | $${pay.amount} | ${pay.methodName}`,
      );
    }
    ...
    if (sampleLayaway) {
      ...
      console.log(
        `  Months: ${plan?.months}, Frequency: ${plan?.paymentFrequency}, Down: $${plan?.downPayment}`,
      );
      ...
      if (plan?.installments.length) {
        for (const inst of plan.installments.slice(0, 3)) {
          console.log(
            `    ${inst.label} - $${inst.amount} due ${inst.dueDate.toLocaleDateString()} ${inst.isPaid ? "(PAID)" : ""}`,
          );
        }
      }
    }
  ...
  console.log(
    `  Created ${customerCount} customers (${customerNames.size - customerCount} already existed)`,
  );
  ...
  for (const pay of payments) {
    ...
    if (!methodMap.has(key)) {
      const created = await prisma.paymentMethodEntry.create({
        data: {
          name: pay.methodName,
          sortOrder: allMethods.length + methodMap.size + 1,
        },
      });
  ...
  for (const inv of invoices) {
    ...
    try {
      ...
      if (failedInvoices <= 10) {
        console.error(
          `  Failed invoice ${inv.externalInvoiceNumber || "unknown"}: ${err.message}`,
        );
      }
    }
  ...
  console.log(
    `  Created ${invoiceCount} invoices (${layawayCount} with layaway plans), ${skippedInvoices} skipped, ${failedInvoices} failed`,
  );
  ...
  for (const pay of payments) {
    const methodId =
      methodMap.get(pay.methodName.toLowerCase()) || methodMap.get("cash")!;
    ...
    try {
      ...
      if (failedPayments <= 10) {
        console.error(
          `  Failed payment for invoice ${pay.invoiceNumber || "unknown"}: ${err.message}`,
        );
      }
    }
  ...
  console.log(
    `  Created ${paymentCount} payments (${matchedCount} matched to invoices), ${failedPayments} failed`,
  );
